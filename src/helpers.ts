...
export function randomTimeRange(
  ...
  const randomValueMs = Number(randomValue.toFixed(DECIMAL_PLACES)) * MS_RATIO;
  return randomValueMs;
}

export function infoWithDate(message: string, fileWriter: FileSink) {
  const messageWithDate = new Date().toISOString() + ": " + message;
  fileWriter.write(messageWithDate + '\n');
  console.info(messageWithDate);
}
